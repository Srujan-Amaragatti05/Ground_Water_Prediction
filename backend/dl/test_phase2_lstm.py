...
from phase2_lstm import preprocess_data, build_model

class TestPhase2LSTM(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.csv_path = 'mock_dataset.csv'
        data = {
            'STATE_UT': ['S1']*12 + ['S2']*7,
            'DISTRICT': ['D1']*12 + ['D2']*7,
            'BLOCK': ['B1']*12 + ['B2']*7,
            ...
            'Date': [
                '2013-01-01', '2014-01-01', '2015-01-01', '2016-01-01', '2017-01-01', '2018-01-01', # Target 2018 (Train)
                '2019-01-01', '2020-01-01', '2021-01-01', '2022-01-01', '2023-01-01', '2024-01-01', # Target 2024 (Excluded)
                '2018-01-01', '2019-01-01', '2020-01-01', '2021-01-01', '2022-01-01', '2023-01-01', '2024-01-01' # Target 2023 (Test)
            ],
        ...
        cls.data, cls.scalers, cls.annual_df = preprocess_data(cls.csv_path)
    ...
    def test_predictions(self):
        ...
        if len(self.data['X_val']) > 0:
            preds = model.predict(self.data['X_val'])
            self.assertFalse(np.isnan(preds).any())
            self.assertFalse(np.isinf(preds).any())
        
    def test_model_save_reload(self):
        model = build_model((5, 4))
        save_path = 'temp_test_model.keras'
        model.save(save_path)
        loaded_model = tf.keras.models.load_model(save_path)
        if len(self.data['X_val']) > 0:
            preds_original = model.predict(self.data['X_val'])
            preds_loaded = loaded_model.predict(self.data['X_val'])
